...
export function ChecklistPanel({ apId }: Props) {
  ...
  const { aps, addChecklistItem, deleteChecklistItem, marcarChecklist } = useFenixStore();

  // Encontra a AP atual e seus checklists
  ...
  const handleToggle = async (item: any) => {
    try {
      await marcarChecklist(item.id, !item.concluido);
  ...
  const handleAddItem = async () => {
    ...
    try {
      await addChecklistItem(novoItem.trim(), apId);
      setNovoItem('');
      setNovaData('');
