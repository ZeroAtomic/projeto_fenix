...
export function APManagerPage() {
  ...
  // Carrega Disciplina
  const disciplina = useLiveQuery(
    () => db.disciplinas.get(Number(disciplinaId)),
    [disciplinaId]
  );

  // Carrega APs
  const apsDestaDisciplina = useLiveQuery(
    () => db.aps.where({ id_disciplina: Number(disciplinaId) }).toArray(),
    [disciplinaId]
  ) || [];

  // Carrega Checklists para calcular progresso
  const checklists = useLiveQuery(
    async () => {
      const apIds = apsDestaDisciplina.map(a => a.id!);
  ...
  const handleAddAP = async () => {
    if (!novaAPTitulo.trim() || !podeAdicionarAP || !disciplinaId) return;
    setIsSaving(true);
  ...
  const handleDeleteAP = async (apId?: number) => {
    ...
    try {
      await db.aps.delete(apId);
      const items = await db.checklist.where({ id_ap: apId }).toArray();
      await db.checklist.bulkDelete(items.map(i => i.id!));
    } catch (err) {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="space-y-4">
          {apsDestaDisciplina.map((ap) => {
            const items = checklists.filter(c => c.id_ap === ap.id);
