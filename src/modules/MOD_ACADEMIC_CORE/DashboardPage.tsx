...
// Dashboard principal com métricas do SQLite via Zustand Store
// ============================================================
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchAps();
    fetchProgresso();
  }, [fetchDisciplinas, fetchAps, fetchProgresso]);

  // Mapeia disciplinas do SQLite (API) para o formato esperado pelo frontend
  ...
  // Progresso do semestre (atividades criadas vs capacidade máxima)
          ...
          {disciplinas.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {disciplinas.slice(0, 4).map(d => {
                const discAps = aps.filter(a => a.disciplinaId === d.id).map(a => a.id!);
                const discItems = checklistItems.filter(c => discAps.includes(c.atividadeId));
