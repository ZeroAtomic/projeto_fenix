...
// Listagem e gestão de disciplinas do aluno
// ============================================================
...
import { db, type Disciplina } from '../../modules/MOD_DB/db';
...
export function DisciplinasPage() {
  ...
  // Reatividade automática do Dexie (Dica 2)
  const disciplinas = useLiveQuery(
    () => db.disciplinas.where({ user_id: Number(userProfile?.id) || 1 }).toArray(),
    [userProfile]
  ) || [];

  const handleSave = async (d: Disciplina) => {
    try {
      const payload = { ...d, user_id: Number(userProfile?.id) || 1 };
      if (editando?.id) {
        await db.disciplinas.put({ ...payload, id: editando.id });
      } else {
        await db.disciplinas.add(payload as Disciplina);
      }
      setEditando(null);
      setModalOpen(false);
    } catch (err) {
  ...
  const handleDelete = async (id?: number) => {
    ...
    try {
      ...
      // Apaga dependências associadas (APs e Checklists)
      const apsDaDisc = await db.aps.where({ id_disciplina: id }).toArray();
      const apIds = apsDaDisc.map(ap => ap.id!);
      await db.aps.bulkDelete(apIds);
      
      const checklists = await db.checklist.where('id_ap').anyOf(apIds).toArray();
      await db.checklist.bulkDelete(checklists.map(c => c.id!));
    } catch (err) { 
